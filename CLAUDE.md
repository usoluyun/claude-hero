...
| [`hero-agent-layers.md`](./docs/hero-agent-layers.md) | agent 分层总图 + 能力矩阵（双轴 + 角色三梯 + 先驱花名，skills/CLI 施工底图） | 看全景分层 / 加新 agent / 查 skill 用在哪 |
...
- **`agents/`** — 共享 subagent（`hero-java-*.md`），按**双轴分层**组织：角色 agent（规划/执行/
  评审门控三梯，横向干活）× 项目领航 agent（按服务只读带路）。完整分层 + 先驱花名 + 能力矩阵见
...
- **新增资源**：新 skill/agent/hook/MCP/CLI 的落位规则见 `CONTRIBUTING.md`；需安装到 `~/.claude` 的
  新类别才改 `manifest.yaml`。
- **`web/` 宣传站（podman 容器化）**：站点源在 `web/public`，由 `web/Containerfile`（caddy:2-alpine）打镜像，**单容器直接 `podman run`**（不走 compose），本机 `http://localhost:10086/` 对外。改完内容 `cd web && bash run.sh` 一键重建重启。**CSS 版本管理**：`web/public/css/*.css` 的引用必须带 `?v=YYYYMMDD<letter>` 做 cache bust（如 `tokens.css?v=20260614b`）。改任何 CSS 后**必须递增所有引用它的 HTML 里的版本号**，否则中间层/浏览器缓存会让用户滞后看到旧样式。Caddyfile 未配 cache-control，全靠 query string 破缓存。详见 [`web/README.md`](./web/README.md)。
