...
| `issue decompose <iid>` | 拆解主 Issue 为子任务 | **tech-lead（孔明）** | ⏸ STOP 确认拆解方案 |
...
命令进入
  │
  ├─ decompose → 强制路由 tech-lead（孔明），不管标签
  │
  ├─ claim / done → 读取 Issue 标签 hero::agent:<name>
  │     ├─ 有标签 → 路由到对应 agent
  │     └─ 无标签 → 路由 tech-lead（孔明）分配
  │
...
|---|---|---|---|
| `kongming` | tech-lead | 孔明 | `agents/hero-java-tech-lead.md` |
| `wenyuan` | backend-dev | 文远 | `agents/hero-java-backend-developer.md` |
| `zichang` | data-engineer | 子长 | `agents/hero-java-data-engineer.md` |
| `xiren` | test-engineer | 希仁 | `agents/hero-java-test-engineer.md` |
| `xuancheng` | code-reviewer | 玄成 | `agents/hero-java-code-reviewer.md` |
| `pengju` | security-auditor | 鹏举 | `agents/hero-java-security-auditor.md` |
| （无标签） | tech-lead | 孔明 | `agents/hero-java-tech-lead.md` |
...
|------|------|-----------|------|-----------|
| **tech-lead** | 孔明 | `hero-java-tech-lead` | 架构设计、任务拆解、分配协调 | `decompose`, `assignment`, `分解`, `分配`, `拆解` |
| **backend-dev** | 文远 | `hero-java-backend-developer` | 业务逻辑实现、中间件接入 | `code`, `implementation`, `代码`, `实现`, `接口`, `Controller`, `Service` |
| **data-engineer** | 子长 | `hero-java-data-engineer` | 数据层、MyBatis、SQL 调优 | `data`, `SQL`, `数据`, `数据库`, `MyBatis`, `Mapper`, `索引` |
| **test-engineer** | 希仁 | `hero-java-test-engineer` | TDD/BDD 测试、接口冒烟、E2E | `test`, `testing`, `测试`, `验证`, `单元测试`, `冒烟`, `Allure` |
| **code-reviewer** | 玄成 | `hero-java-code-reviewer` | 代码审查（只读） | `review`, `code review`, `代码审查`, `评审`, `质量` |
| **security-auditor** | 鹏举 | `hero-java-security-auditor` | 安全审计（只读） | `security`, `audit`, `安全`, `审计`, `漏洞`, `越权` |
...
🦸 hero ▸ 孔明（hero-java-tech-lead）接手 · 任务拆解

→ 孔明读取 Issue #123（glab issue view 123）
→ 孔明分析需求，规划子任务：
   - 子任务 A：数据库表设计 → 子长（data-engineer）
   - 子任务 B：接口实现 → 文远（backend-dev）
   - 子任务 C：单元测试 + 冒烟 → 希仁（test-engineer）

⏸ STOP ① 「确认拆解方案」

| 子任务 | 负责人 | 优先级 | 标签 |
|--------|--------|--------|------|
| 数据库表设计 | 子长 | high | hero::agent:zichang |
| 接口实现 | 文远 | high | hero::agent:wenyuan |
| 测试 | 希仁 | medium | hero::agent:xiren |
...
🦸 hero ▸ 文远（hero-java-backend-developer）接手 · 认领任务

→ 文远认领：glab issue update 124 --label "hero::status:in_progress" --unlabel "hero::status:pending"
→ 文远开始工作...
────────────────────────────────────────
...
🦸 hero ▸ 读取 Issue #124 标签 → hero::agent:wenyuan
🦸 hero ▸ 文远（hero-java-backend-developer）接手 · 完成汇报
...
→ 文远更新状态：glab issue update 124 --label "hero::status:done" --unlabel "hero::status:in_progress"

→ ⚠️ 关闭前检查标签 → 确认是 hero::type:task，非 epic
→ 文远关闭：glab issue close 124

→ 文远检查父 Issue #123 的子任务完成情况：
  glab issue list --output json | jq '[.[] | select(.description | contains("Relates to #123"))]'
  → 发现还有 #125（希仁/测试）未完成
  → 提示用户：「#124 已完成。父 Issue #123 还有子任务 #125（希仁/测试）待完成。」
```
...
→ 按优先级排序，逐个认领并路由：
  ...
  🦸 hero ▸ 子长（hero-java-data-engineer）接手 · Issue #201（hero::priority:high）
  🦸 hero ▸ 文远（hero-java-backend-developer）接手 · Issue #202（hero::priority:medium）
  🦸 hero ▸ 希仁（hero-java-test-engineer）接手 · Issue #203（hero::priority:low）
...
|-------|---------|-------------|------|------|
| 文远（backend-dev） | 3 | 1 | 5 | 9 |
| 子长（data-engineer） | 1 | 0 | 2 | 3 |
| 希仁（test-engineer） | 2 | 1 | 3 | 6 |
| 玄成（code-reviewer） | 0 | 0 | 4 | 4 |
| 鹏举（security-auditor） | 0 | 0 | 1 | 1 |
