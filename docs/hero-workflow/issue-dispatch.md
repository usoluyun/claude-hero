...
命令进入
  ...
  │
  ├─ review → 强制路由 code-reviewer（玄成），只读
  │
  ├─ audit → 强制路由 security-auditor（鹏举），只读
  │
...
|---|---|---|---|
| `kongming` | 孔明 | tech-lead | `agents/hero-java-tech-lead.md` |
| `wenyuan` | 文远 | backend-dev | `agents/hero-java-backend-developer.md` |
| `zichang` | 子长 | data-engineer | `agents/hero-java-data-engineer.md` |
| `xiren` | 希仁 | test-engineer | `agents/hero-java-test-engineer.md` |
| `xuancheng` | 玄成 | code-reviewer | `agents/hero-java-code-reviewer.md` |
| `pengju` | 鹏举 | security-auditor | `agents/hero-java-security-auditor.md` |
| （无标签） | 孔明 | tech-lead | `agents/hero-java-tech-lead.md` |
...
③ SKILL.md 路由表
    ...
    ├─ hero::agent:wenyuan  → 路由到文远（backend-dev）
    ├─ hero::agent:zichang  → 路由到子长（data-engineer）
    └─ 无标签              → 降级到孔明（tech-lead）
    │
    ▼
④ 文远认领，更新状态
    glab issue update 456 --label "hero::status:in_progress"
    输出确认：「🦸 hero ▸ 文远（hero-java-backend-developer.md）接手 · 已认领 #456，开始工作」
```

同理，`issue done #456 "修复完成"` 会走相同的路由逻辑，由对应 agent 执行完成汇报和关闭操作。`issue decompose #1` 则不管 Issue 标签，强制交给孔明（tech-lead）拆解为子任务。
...
| `issue decompose <iid>` | 拆解为子任务 | 强制孔明（tech-lead） |
| `issue list <agent>` | 列出 agent 待办 | 直接查询 |
| `issue view <iid>` | 查看详情 | 直接查看 |
| `issue status` | 全局任务分布 | 直接统计 |
| `issue review <iid>` | 代码审查（只读） | 强制玄成（code-reviewer） |
| `issue audit <iid>` | 安全审计（只读） | 强制鹏举（security-auditor） |
